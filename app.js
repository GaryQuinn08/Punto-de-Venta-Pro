...
function showPage(pageKey) {
  ...
  if (pageKey === 'inv')  renderInventory();
}
...
function renderProducts(list) {
  ...
  grid.innerHTML = list.map(p => {
    ...
    const stLabel = p.stock <= 0 ? 'Sin stock' : 'Stock: ' + p.stock;
    return `<div class="product-card${inCart}" onclick="addToCart(${p.id})">
      <div class="prod-name">${p.name}</div>
      <div class="prod-price">$${p.price.toFixed(2)}</div>
      <div class="prod-stock ${stClass}">${stLabel}</div>
    </div>`;
  }).join('');
}
...
function updateCart() {
  ...
  calcChange();
  filterProducts(); // refresh in-cart highlights
}
...
function showTicketModal(sale) {
  const rows = sale.items.map(i => {
    const nameTrim = i.name.length > 32 ? i.name.substring(0, 32) + '…' : i.name;
    return `<div class="ticket-row">
      <span>${nameTrim} x${i.qty}</span>
...
function skipPrint() {
  showToast('Venta registrada sin imprimir ✓');
  startNewSale();
}
...
function renderInventory() {
  ...
  tbody.innerHTML = list.map(p => {
    ...
    return `<tr id="inv-row-${p.id}">
      <td style="color:var(--subtle);font-size:11px">${p.id}</td>
...
function toggleEditRow(id) {
  ...
  const isSaving = btn.classList.contains('save');
  ...
  if (isSaving) {
    ...
    priceDisp.style.display = '';
    priceInp.style.display  = 'none';
    stockDisp.style.display = '';
    ...
    priceInp.style.display  = 'inline-block';
    stockDisp.style.display = 'none';
    stockInp.style.display  = 'inline-block';
